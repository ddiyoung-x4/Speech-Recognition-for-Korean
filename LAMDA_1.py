...
if __name__ == '__main__':
    wav_list = os.listdir("./문제1")

    answer = []
    for wav_file in wav_list:
        a = {}
        a['filename'] = wav_file
        frames, rate = get_duration("./문제1/"+wav_file)
        a['duration'] = math.floor(frames / rate * 1000)/1000

        # logging.basicConfig(level=logging.DEBUG)

        it = iter(WavFile("./문제1/"+wav_file))
        text_data = ""
        audio_data = 0.0
        while True:
            try:
                x = next(it)
                if x.name.decode('ascii') == 'THIS':
                    try:
                        # print(x.data.decode('utf-8'))
                        text_data = x.data.decode('utf-8')
                        a['THIS'] = text_data
                    except UnicodeDecodeError:
                        # print('frame_size', x.size / 2)
                        audio_data = (x.size / 2) / rate
                        a['THIS'] = math.floor(audio_data * 1000) / 1000
